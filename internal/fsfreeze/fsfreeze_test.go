...
`[1:], dir)
	...
	thaw, result, err := s.freezeLocalFilesystems()
	c.Check(err, check.ErrorMatches, "cannot freeze: operation not supported")

	if err == nil {
		thaw.Close()
		c.Assert(<-result, check.ErrorMatches, "cannot freeze: operation not supported")
	}
...
`[1:], dir)
	...
	s.frozen[dir] = errors.New("freeze not permitted")
	close(s.canThaw)

	thaw, result, err := s.freezeLocalFilesystems()
	c.Check(err, check.ErrorMatches, "cannot freeze: freeze not permitted")

	if err == nil {
		thaw.Close()
		c.Assert(<-result, check.ErrorMatches, "cannot freeze: freeze not permitted")
	}

	c.Check(s.frozen, check.HasLen, 1)
	c.Check(s.frozen[dir], check.ErrorMatches, "freeze not permitted")
}
...
`[1:], dir)
	...
	c.Check(<-result, check.ErrorMatches, "timed out waiting for thaw notification")
	thaw.Close()
