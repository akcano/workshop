...
func FreezeLocalFilesystems(stdin io.Reader, stdout io.Writer) (err error) {
	...
	defer func() {
		err = errors.Join(err, thaw(frozen))
	}()
	...
	ctx, cancel := context.WithTimeoutCause(ctx, timeout, errors.New("timed out waiting for thaw notification"))
	defer cancel()
	...
	go func() {
		defer func() {
			// Recover to ensure thaw() is called, but don't bother unwrapping
			// the error message since this is unlikely in practice.
			if recover() != nil {
				cancelCause(errors.New("panic: cannot read standard input"))
			}
		}()
...
func localMounts(mountinfo, filesystems string) ([]*osutil.MountInfoEntry, superblock, error) {
	...
	if err != nil {
		if _, ok := errors.AsType[*os.PathError](err); !ok {
			err = fmt.Errorf("invalid %q entry: %w", mountinfo, err)
		}
		return nil, superblock{}, err
	}
...
func freeze(ctx context.Context, mounts []*osutil.MountInfoEntry, root superblock) (files []*os.File, err error) {
	rev := revert.New()
	defer rev.Fail()

	var frozen []*os.File
	rev.Add(func() {
		errs := make([]error, 1, len(frozen)+1)
		errs[0] = err
		for _, f := range slices.Backward(frozen) {
			errs = append(errs, fsThaw(f))
			f.Close()
		}
...
func thaw(frozen []*os.File) error {
	errs := make([]error, 0, 2*len(frozen))
